#!/usr/bin/env python3
"""
run_term_extractor.py

Unified entry point for HF-based and LLM-based biomedical term extraction.
...
Dispatch matrix
---------------
    extractor: hf  + mode: train -> HFTermExtractor.run_training()
    extractor: hf  + mode: inference -> HFTermExtractor.run_inference()
    extractor: llm -> LLMTermExtractor.run_inference() (LLMs are inference-only for now)

Usage
-----
    # Basic run
    python run_term_extractor.py --config scripts/configs/hf_train.yaml

    # With CLI overrides (dot-notation, override any YAML key)
    python run_term_extractor.py --config scripts/configs/llm_lmstudio.yaml \\
...
def apply_overrides(config: dict, overrides: list[str]) -> dict:
    ...
    Dot notation descends into nested dicts:
        ...
        "extractor=llm" -> config["extractor"] = "llm"
...
# Namespace builders #
######################
# These functions translate the YAML config dict into the argparse.Namespace objects that HFTermExtractor and LLMTermExtractor expect.
...
def build_hf_namespace(config: dict) -> argparse.Namespace:
    """
    Translates the 'hf' section of the config dict into an argparse.Namespace compatible with HFTermExtractor.run_training() and run_inference().

    All keys mirror the CLI flags documented in HFTermExtractor.parse_args().
    ...
    if mode not in ("train", "inference"):
        raise ValueError(
            f"Invalid mode '{mode}' for extractor 'hf'.  "
    ...
    ns = argparse.Namespace(
        ...
        eval_data_path   =_get(hf, "eval_data_path"),
        ...
        # The YAML key is 'separate_title_abstract'; the derived flag used
        # internally by the module is 'concatenate_title_abstract'.
    ...
    # -- Validation (mirrors HFTermExtractor.parse_args()) --
...
def build_llm_namespace(config: dict) -> argparse.Namespace:
    """
    Translates the 'llm' section of the config dict into an argparse.Namespace compatible with LLMTermExtractor.run_inference().
    All keys mirror the CLI flags documented in LLMTermExtractor.parse_args().
    ...
    ns = argparse.Namespace(
        ...
        prompts_path         =_get(llm, "prompts_path", "../data/prompts.json"),
...
def dispatch(config: dict) -> None:
    """
    Reads the top-level 'extractor' and 'mode' keys from the config and calls the appropriate extractor entry point.

    :param config: Fully merged and validated config dict.
    :raises ValueError: If 'extractor' is not 'hf' or 'llm'.
    """
    extractor = config.get("extractor", "").lower()

    if extractor == "hf":
        # Import lazily: users who only use LLM extraction do not need torch / HF
        from src.termExtractor.HFTermExtractor import run_training, run_inference

        ns = build_hf_namespace(config)
        if ns.inference_only:
            print("[main] Dispatching -> HFTermExtractor / inference")
            run_inference(ns)
        else:
            print("[main] Dispatching -> HFTermExtractor / training")
            run_training(ns)

    elif extractor == "llm":
        from src.termExtractor.LLMTermExtractor import run_inference

        ns = build_llm_namespace(config)
        print(
            f"[main] Dispatching -> LLMTermExtractor / inference "
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Unified term extraction runner.  "
            "Pass a YAML config file; optionally override individual keys inline."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
examples:
    python run_term_extractor.py --config configs/hf_train.yaml
    python run_term_extractor.py --config configs/llm_lmstudio.yaml --override llm.temperature=0.0
    python run_term_extractor.py --config configs/hf_inference.yaml --override hf.model_path=runs/best
        """,
    )
...
if __name__ == "__main__":
    ...
    # 4. Dispatch to the correct extractor
    dispatch(config)