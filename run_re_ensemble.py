...
YAML-driven entry point for relation-level prediction ensembling.

Usage
-----
    python run_re_ensemble.py --config scripts/configs/relationExtractor/ensemble.yaml
    python run_re_ensemble.py --config scripts/configs/relationExtractor/ensemble.yaml --override method=union
"""
...
from src.inference.re_ensemble import SUPPORTED_ENSEMBLE_METHODS, run_inference
...
def build_namespace(config: dict) -> argparse.Namespace:
    ...
    ns = argparse.Namespace(
        ...
        relation_key=_get(config, "relation_key", "relations"),
    )
...
def parse_args() -> argparse.Namespace:
    """
    Parses command-line arguments for run_re_ensemble.py.

    :return: An argparse.Namespace with 'config' and 'override' attributes.
    """
    parser = argparse.ArgumentParser(
        description=(
            "Relation ensemble runner. Pass a YAML config file; optionally "
...
if __name__ == "__main__":
    ...
    print(f"[main] Prepared {len(jobs)} relation ensemble job(s).")
    for job_index, namespace in enumerate(jobs, start=1):
        print(
            f"[main] Dispatching {job_index}/{len(jobs)} -> "
            f"EnsembleREInference / {namespace.method} / "
