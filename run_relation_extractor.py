#!/usr/bin/env python3
"""
run_relation_extractor.py

Unified entry point for HF-based and LLM-based biomedical relation extraction.

Reads a YAML configuration file, optionally applies dot-notation key=value overrides supplied on the command line, and dispatches to the appropriate extractor module and run mode.

Dispatch matrix
---------------
    mode: train -> HFRelationExtractor.run_training()
    mode: inference -> HFRelationExtractor.run_inference() or LLMRelationExtractor.run_inference()

Usage
-----
    # HF training
    python run_relation_extractor.py --config scripts/configs/relation_train.yaml

    # LLM inference
    python run_relation_extractor.py --config scripts/configs/relation_llm_inference.yaml
...
Config schema
-------------
See the YAML files under scripts/configs/ for annotated examples of every field.
The top-level 'extractor' and (for hf) 'mode' keys control dispatch; all other parameters live under the 'hf' or 'llm' sub-key.
"""
...
# Namespace builders #
######################
# These functions translate the YAML config dict into the argparse.Namespace objects that HFRelationExtractor expects.
...
def build_hf_namespace(config: dict) -> argparse.Namespace:
    """
    Translates the 'hf' section of the config dict into an argparse.Namespace compatible with HFRelationExtractor.run_training() and run_inference().

    All keys mirror the CLI flags documented in HFRelationExtractor.parse_args().
    ...
    if mode not in ("train", "inference"):
        raise ValueError(
            f"Invalid mode '{mode}' for relation extraction.  "
    ...
    ns = argparse.Namespace(
        ...
        batch_size   =_get(hf, "batch_size",    16),
        num_epochs   =_get(hf, "num_epochs",    1),
        ...
        concatenate_title_abstract=_get(hf, "concatenate_title_abstract", False),
    )

    # -- Validation (mirrors HFRelationExtractor.parse_args()) --
...
def build_llm_namespace(config: dict) -> argparse.Namespace:
    """
    Translates the 'llm' section of the config dict into an argparse.Namespace compatible with LLMRelationExtractor.run_inference().

    All keys mirror the CLI flags documented in LLMRelationExtractor.parse_args().
    ...
    ns = argparse.Namespace(
        ...
        prompts_path         =_get(llm, "prompts_path", "src/relationExtractor/prompts.json"),
        ...
        # -- Relation extraction specific --
        concatenate_title_abstract=_get(llm, "concatenate_title_abstract", False),
    )
...
def dispatch(config: dict) -> None:
    """
    Reads the top-level 'extractor' and (for hf) 'mode' keys from the config and calls the appropriate relation extractor entry point.

    :param config: Fully merged and validated config dict.
    :raises ValueError: If 'extractor' is not 'hf' or 'llm'.
    """
    extractor = config.get("extractor", "hf").lower()

    if extractor == "hf":
        # Import lazily: users who only use LLM extraction do not need torch / HF
        from src.relationExtractor.HFRelationExtractor import run_training, run_inference

        ns = build_hf_namespace(config)
        if ns.inference_only:
            print("[main] Dispatching -> HFRelationExtractor / inference")
            run_inference(ns)
        else:
            print("[main] Dispatching -> HFRelationExtractor / training")
            run_training(ns)

    elif extractor == "llm":
        from src.relationExtractor.LLMRelationExtractor import run_inference

        ns = build_llm_namespace(config)
        print(
            f"[main] Dispatching -> LLMRelationExtractor / inference "
        ...
        raise ValueError(
            f"Unknown extractor '{extractor}' in config.  "
...
def parse_args() -> argparse.Namespace:
    """
    Parses command-line arguments for run_relation_extractor.py.
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Unified relation extraction runner (HF or LLM).  "
            "Pass a YAML config file; optionally override individual keys inline."
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
examples:
    python run_relation_extractor.py --config configs/relation_hf_train.yaml
    python run_relation_extractor.py --config configs/relation_hf_train.yaml --override hf.num_epochs=5
    python run_relation_extractor.py --config configs/relation_hf_train.yaml --override hf.concatenate_title_abstract=true
    python run_relation_extractor.py --config configs/relation_llm_inference.yaml
    python run_relation_extractor.py --config configs/relation_llm_inference.yaml --override llm.temperature=0.0
        """,
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Example: --override hf.learning_rate=1e-5 hf.num_epochs=3"
        ),
    )
...
if __name__ == "__main__":
    ...
    # 4. Dispatch to the relation extractor
    dispatch(config)
